...
fn if_else<W: Write>(stmt_id: StmtId, id: IfElseId, ast: &Ast, writer: &mut BufWriter<W>) -> Result<()> {
    let stmt = &ast[id];
    ...
    writeln!(writer, "\"{stmt_id:?}\" -> \"{:?}\"", stmt.if_branch.global_id())?;
    graph(stmt.if_branch, ast, writer)?;
